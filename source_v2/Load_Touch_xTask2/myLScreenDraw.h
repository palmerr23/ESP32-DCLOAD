...
short tMode(short sv){
	...
	if (sv == TBUT)
	{
	  if (pSetX.trackOn)
	  {
		if(pSetX.eTrack <= 1.0 - SMALL_DIFF)
			return BBACT;
		else
			return BBINACT;
	  }
...
void drawIndicators(short aMenu)
{
	...
	tft.setCursor(sx(READX + 90), sy(TOPLEGS));
	...
	tft.setCursor(sx(READX + 110), sy(15));
	tft.print("E");
  ...
  switch (aMenu)
  {
	...
	if (!notSTmode) // in TR mode main settings are hidden
	{
		...
		{
		  	tft.setCursor(sx(xstart + ABUTOFF), sy(ystart - ABUTOFF));
			tft.print("L");
		}
		*/
		...
		/*
		xcol = BGCOL;
		if (_curPWM > FANIND) xcol = IND_ONLC;	
		if (_curPWM > FANIND2) xcol = IND_OFFC;	
		tft.fillRoundRect(sx(VALX-8), sy(6), sx(8), sx(8),  3, xcol); 
		*/
	/*
		...
		if (tm == BBACT)
			xcol = (myBlink)? xcol : BGCOL;	
	    tft.fillRoundRect(sx(ABUTX), sy(BVSPACE) - ABUT, ABUT, ABUT,  ABUTR, xcol); 
		if(tm > 0)
		{
			tft.setCursor(sx(ABUTX + ABUTOFF), sy(BVSPACE - ABUTOFF));
			tft.print("T");
		}		
		*/
		// under current 
		/*
		xcol = (_outOn && (pSet.current - meas.ADSamps) > MEDIUM_DIFF) ? MY_ORANGE : BGCOL;	
		tft.fillRoundRect(sx(40), sy(TOPLINE/2+1*BVSPACE), sx(8), sx(8),  3, xcol); 
		*/
		// under voltage - which modes?
		//xcol = (_outOn && (pSet.voltage - meas.ADSvolts  > MEDIUM_DIFF) && pSet.mode != MODE_CC) ? MY_ORANGE : BGCOL;	
		//tft.fillRoundRect(sx(40), sy(TOPLINE+0*BVSPACE), sx(8), sx(8),  3, xcol); 
...
// fmt: 'F' print as float; 'I' & 'U' - unsigned 8 and 16 bit int (decimal justified); 'P' - float as percentage, 'B' - bracketed Int; 'T' = On/off (1/0; T > 0.1); 'S' = String; 'A' = safe string (no unprintable chars)
void printX(valFmt * xp,  short highlight, uint16_t txtcol, uint16_t highcol, uint16_t bgcol, bool editing)
{  
...
void screenError(char * message, uint16_t bgcol, uint16_t onTime, bool logo)
{
   ...
   tft.setRotation(SCREENROT);
	 ...
	 {
		// may need to feed wdt
		feedLoopWDT();	// Just in case
		delay(1); // allow other xTasks (WiFi) to proceed
		yield();
	 }
	 dirtyScreen = true;
}
