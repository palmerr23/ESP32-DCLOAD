...
#define BBORD BGCOL
#define BBGHIGH RGB565(0,14,28)
#define BBXHIGH RGB565(0,0,20)
// mode buttons
#define BONCOL MY_WHITE
#define BBMHIGH RGB565(0,45,0)
#define BBMXHIGH RGB565(3,20,0)
// cal buttons
...
struct screenCals {
  ...
  int16_t tvmax;
  int16_t tsRot;	// screen rotation - as some touch screens are rotated 180%
} ;
screenCals sc = {400, 300, 3900, 3700, TOUCHROT}; // useful starting values, overwritten by screen cal and EEPROM restore.
...
#define SV_P 2
   ...
   {&ltSetting, 2, 1, 0,65535, sx(READX+12),sy(TOPLINE+4*BVSPACE), 'U', "", RES_COL, 0,5,0, NULL},    // log time, always show
#define SET_COM 5
   //COMMS MENU - 1
...
#define SET_CAL (SET_STEP + 10)
   ...
   {&calOffLo, 2, 1,-30, 30, sx(READX),sy(TOPLINE+2*BVSPACE-5), 'F', "", TEMP_COL,3,2,1, NULL},   
   {&calOffHi, 2, 1,-30, 30, sx(READX),sy(TOPLINE+3*BVSPACE-5), 'F', "", TEMP_COL,3,2,1, NULL},   
...
#define RV_I 1
   ...
   {&tsRota,   2, 1,-1,4,   HMAX/2 - 18, VMAX/2 + sy(50), 'I', "", LIGHTYELLOW,3,1,0, NULL},  // screen rotation
...
legend leg[NUMLEGS] = {
   //{0,  1,sx(READX), sy(TOPLEGS), 0, "SET", HIGHLIGHTCOL, NULL},
   //{0,  1,sx(LEGX+20), sy(TOPLINE+2.5*BVSPACE), 0, "Track", CYAN_L, &iSTrack},
  // {0,  1,sx(VALX+25), sy(TOPLINE+2.5*BVSPACE), 0, "Grp", CYAN_L, &iSTrack},
   // COMMS MENU - 1
   ...
   //{1,  1,LEG2X, TOPLINE, 1, "IP", CYAN_L, NULL},
   //{2,  1,VAL2X+IPDOT, TOPLINE+5, 1, ".", CYAN_L, NULL}, // dots between IP, Y+5 avoids block erase on value edit
  // {2,  1,VAL2X+IPDOT+IPSPACE, TOPLINE+5, 1, ".", CYAN_L, NULL},
  // {2,  1,VAL2X+IPDOT+2*IPSPACE, TOPLINE+5, 1, ".", CYAN_L, NULL},
   // TRACK MENU - 2
  ...
  // {0,  1,LEG2X, TOPLEG+BVSPACE2, 3, "Instrument", CYAN_L, NULL},
   //{0,  1,CAL2X+10, TOPLEG+BVSPACE2, 3, "Ext reading", CYAN_L, NULL},
  // {2,  1,LEG2X, TOPLINE+0*BVSPACE2+BVSPACE2/2, 3, "V", CYAN_L, NULL},
  // {0,  1,LEG2X+30, TOPLINE+0*BVSPACE2, 3, "Hi", CYAN_L, NULL},
  // {0,  1,LEG2X+30, TOPLINE+1*BVSPACE2, 3, "Lo", CYAN_L, NULL},
   //{2,  1,LEG2X, TOPLINE+2*BVSPACE2+BVSPACE2/2, 3, "A", CYAN_L, NULL},
  // {0,  1,LEG2X+30, TOPLINE+2*BVSPACE2, 3, "Hi", CYAN_L, NULL},
  // {0,  1,LEG2X+30, TOPLINE+3*BVSPACE2, 3, "Lo", CYAN_L, NULL},
//{0,  1,LEG2X, TOPLINE+4*BVSPACE2, 3, "In Hi", CYAN_L, NULL},
  // {0,  1,LEG2X, TOPLINE+5*BVSPACE2, 3, "V  Lo", CYAN_L, NULL},
...
int setNORb(int);
int setZeroCal(int);
//int toggleTriangle(int);
//int toggleOffAfter(int);
...
button but[NUMBUTS] = {
  ...
  {NULL,0, 4, NOSETVAL, 6, sx(READX), sy(0), false,"SET", BBGHIGH, BBXHIGH, NULL}, // SET menu  
  ...
  {&startST,  0, 0, NOSETVAL, 7, sx(90),VMAX - BUTSIZ, false,"ST", BBMHIGH, BBMXHIGH, &isModeB[MODE_ST], NULL}, // same as BAT
  {&setNORb,  0, 0, NOSETVAL, 7, sx(150), VMAX - BUTSIZ, false,"NOR", BBMHIGH, BBMXHIGH, &isModeB[MODE_NOR], NULL},	// bright Green when active
// Settings menu
  {NULL,4, 2, NOSETVAL, 7, (HMAX - BUTSIZ)/2, sy(BTOP+0.5*BVSPACEX), false,"STEP", BBGHIGH, BBXHIGH, NULL, NULL}, // STEP
  {&calEntry,4, 3, NOSETVAL, 7, (HMAX - BUTSIZ)/2, sy(BTOP+2*BVSPACEX), false,"CAL", BBGHIGH, BBXHIGH, NULL, NULL}, //CAL
  {NULL,4, 1, NOSETVAL, 7, (HMAX - BUTSIZ)/2, sy(BTOP+3.5*BVSPACEX), false,"COM", BBGHIGH, BBXHIGH, NULL, NULL}, // COM
 ...
 // CAL MENU - 3 Volts/amps
  ...
  {&setZeroCal,3, 3, NOSETVAL, 7,(HMAX - BUTSIZ)/2-12, sy(BTOP+BVSPACEX/2), 	false,	"ZERO" , BBGHIGH, BBXHIGH, NULL, NULL}, 
  {&calSave,3, 0, NOSETVAL, 7, 	 (HMAX - BUTSIZ)/2-12, VMAX - BUTSIZ, false,"SAVE" , BBGHIGH, BBXHIGH, NULL, NULL}, // save and exit
    // SCREEN CAL  
  {&screenCal,3, 3, NOSETVAL, 7, sx(20), VMAX - BUTSIZ, 		false,	"TCH", BBGHIGH, BBXHIGH, NULL, NULL}, 
  {&tsRotate,3, 3, NOSETVAL, 7, (HMAX - BUTSIZ)/2-12, (VMAX - BUTSIZ)/2+10, false,"ROT" , BBGHIGH, BBXHIGH, NULL, NULL}, // in middle of screen - as rotation mat be reversed
