...
{
	int ADCval = 0;
  int minC = 1000000; // well outside real values
  int maxC = -1000000;
  int espTemp[ESPREADS];
	/*  don't need great accuracy */
	analogRead(ADC_BV); // throw away the first one
// long tim = micros(); 
	for (int i = 0; i < ESPREADS; i++)
  {
     espTemp[i] = analogRead(ADC_BV);
     ADCval += espTemp[i];
     if(espTemp[i] < minC) minC = espTemp[i];
     if(espTemp[i] > maxC) maxC = espTemp[i];
  }
  ADCval = (ADCval - maxC - minC)/(ESPREADS-2); // discard hi & lo values
 // if(!doneOnce) {Serial.printf("***** ESP reads = %i uS\n", (tim2 - tim)/ESPREADS); doneOnce = true;}
...
inline float readESP_I(void)
{
  ...
  //long tim = micros(); 
	for (i = 0; i < ESPREADS; i++)
  {
     espTemp[i] = analogRead(ADC_BI);
	   ADCval += espTemp[i];
     if(espTemp[i] < minC) minC = espTemp[i];
     if(espTemp[i] > maxC) maxC = espTemp[i];
  }
 ...
 /* 
  ...
  bool var = false;
  #define VAR 50
  for (i = 1; i < ESPREADS; i++)  
    if(abs(espTemp[i] - ADCval) > VAR)
     var = true;
  if(var) 
  { 
    printf("ESP_I avg %i: ", ADCval);
    for (i = 0; i < ESPREADS; i++) 
      //if(abs(ADCval - espTemp[i]) > VAR/2)
        printf ("%i[%i], ", espTemp[i], ADCval - espTemp[i]);
    printf("\n");
  }    
*/
	//Serial.printf(" [%i] ",ADCval);
...
#ifdef THREE_POINT
	if(amps < RDACMID_I)
		dacVal = fmap(amps, halCal[DAC].lowCalVal, halCal[DAC].midCalVal, halCal[DAC].lowCalCount, halCal[DAC].midCalCount);
	else
		dacVal = fmap(amps, halCal[DAC].midCalVal, halCal[DAC].hiCalVal, halCal[DAC].midCalCount, halCal[DAC].hiCalCount);
#else
  dacVal = fmap(amps, halCal[DAC].lowCalVal, halCal[DAC].hiCalVal, halCal[DAC].lowCalCount, halCal[DAC].hiCalCount);
#endif 
