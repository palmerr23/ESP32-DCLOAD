...
void onOffKill(int8_t channel, bool status); // myControl.h
...
int8_t limIndDum, trkIndDum; // dummies for old control values
#define LXIND limIndDum
#define TRXIND trkIndDum
#define OUTXIND _outOn
char DUMMS[1] = "";
valFmt localVals[]
= {
	 {&TRXIND,1, 1, 0,24, READX,TOPLINE+0*BVSPACE, 'Z', "",CYAN_L, 0,2,2} ,    // avoid "units" for type Z
	 {&LXIND, 1, 1,0,1, READX+20,TOPLINE+2*BVSPACE, 'Z', "", CYAN_L,0,2,0} ,  
	 {&OUTXIND, 1, 1,0,1, READX+20,TOPLINE+2*BVSPACE, 'Z', "", CYAN_L,0,2,0},  
	 {&DUMMS, 1, 1,0,1, READX+20,TOPLINE+2*BVSPACE, 'A', "", CYAN_L,0,2,0}   // dummy entry (blank)
};
//long mycountrr;
...
String tableItem(short row, short col)
{
	...
	fmt = ptr->fmt;
	vp = ptr->vp;
	fptr = myFonts[ptr->font]; // may be NULL - match these with known font pointers
	...
	switch (fmt)
	{
	   ...
	   case 'A' : // SAFE string text (may have characters outside 0x20 .. 0x7e range)
		...
		case 'I': // 8-bit Integer: convert value to float for display
		  ...
		  break;
		case 'U': // 16-bit Integer: convert value to float for display
		  spt += itoa( *(uint16_t*)vp, buf, 10);      
		  break;
    case 'L': // 32-bit Integer: convert value to float for display
      spt += itoa( *(uint32_t*)vp, buf, 10);      
      break;
		case 'F': // Float
