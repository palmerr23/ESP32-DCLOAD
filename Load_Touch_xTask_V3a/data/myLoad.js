    ...
    .then(function (myJson) {      
	  ...
	  if("device" in myJson)
	  {
		...
		if("KEL" in myJson)
		{
			...
			//console.log("KEL",ss);
			ll = "K" + i;
			sb = document.getElementById(ll);
			sd = document.getElementById("DK"+i);
			if(ss == i)
				{
					sb.style.background = "green";
					sd.style.background = "green";
				}
			else
				{
					sb.style.background = "#aaa";
					sd.style.background = "#aaa";
				}
			}
		}
		// kelvin sensing setting
		if("KMODE" in myJson)
		{
			ss = myJson["KMODE"];
			for(i of ["A", "M",  "K"]) 
			{
			//console.log("KMODE", ss);
			ll = "SK" + i;
			sb = document.getElementById(ll);
			if(ss == i)
				sb.style.color = "#ff8";
...
function setButtonColor(con)
{
   ...
   // labEl = document.getElementById(lab);		// label
	...
	{
		//	console.log("UN-CHK");
	  divEl.style.background = "#aaa";
	  spaEl.style.background = "#aaa";
