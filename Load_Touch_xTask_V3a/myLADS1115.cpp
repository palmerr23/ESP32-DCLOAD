/**************************************************************************/
/*!
************* Removed m_bitshift code as last 4 bits were being lost on ADS1115
    From:  Adafruit_ADS1015.cpp v1.1
    @author   K.Townsend (Adafruit Industries)
	
   begin() sets I2C clock
...
Adafruit_ADS1015::Adafruit_ADS1015(uint8_t i2cAddress) {
  ...
  m_ads1115 = false;
  m_rate =  ADS1015_REG_CONFIG_DR_1600SPS; // compatible with adafruit
  //m_bitShift = 4;
...
Adafruit_ADS1115::Adafruit_ADS1115(uint8_t i2cAddress) {
  ...
  m_rate = ADS1115_REG_CONFIG_DR_128SPS; // compatible with adafruit
  m_ads1115 = true;
  //m_bitShift = 0;
  m_gain = GAIN_TWOTHIRDS; /* +/- 6.144V range (limited to VDD +0.3V max!) */
}
...
*/
/**************************************************************************/
void Adafruit_ADS1015::begin(uint32_t clk) { 
	Wire.begin(); 
	Wire.setClock(clk);
}
/**************************************************************************/
/*!
    @brief  Sets the conversion time (SPS)

    @param sRate enum of conversion rate (SPS) register setting
*/
/**************************************************************************/
void Adafruit_ADS1015::setRate(uint8_t sRate) {	
	if (m_ads1115)
	{
		sRate = constrain(sRate, 0, 7);
		m_conversionDelay = m_1conv[sRate];		
	}
	else
	{
		sRate = constrain(sRate, 0, 6);
		m_conversionDelay = m_0conv[sRate];			
	}		
	m_rate = sRate << 5; 
}
...
int16_t Adafruit_ADS1015::readADC_Differential_0_1() {
  ...
  uint16_t config =
      ...
      ADS1015_REG_CONFIG_CPOL_ACTVLOW | // Alert/Rdy active low   (default val)
      ADS1015_REG_CONFIG_CMODE_STAD |   // Traditional comparator (default val)
      m_rate |   // 1600 samples per second (default)
      ADS1015_REG_CONFIG_MODE_SINGLE;   // Single-shot mode (default)
...
// ADS1115 values - need to check high threshold for ADS1015
 ...
 void Adafruit_ADS1015::adsStartSingleRdy(uint16_t adsConfig, uint8_t adsMux){

	// SET up COMP registers for interrupt on conversion complete
	// polarity of signal is in adsConfig
	if (mFirst) {
	...
	// could be done by shifting adsMux
  switch (adsMux) 
  {
	  case (0):
		adsConfig = ADS1015_REG_CONFIG_MUX_SINGLE_0 | adsConfig;
		break;
	  case (1):
		adsConfig = ADS1015_REG_CONFIG_MUX_SINGLE_1 | adsConfig;
		break;
	  case (2):
		adsConfig = ADS1015_REG_CONFIG_MUX_SINGLE_2 | adsConfig;
		break;
	  case (3):
		adsConfig = ADS1015_REG_CONFIG_MUX_SINGLE_3 | adsConfig;
		break;
  }
	// start single shot conversion 
	writeRegister(m_i2cAddress, ADS1015_REG_POINTER_CONFIG, adsConfig | ADS1015_REG_CONFIG_OS_SINGLE);
}

 void Adafruit_ADS1015::adsStartSingleDiffRdy(uint16_t adsConfig, uint8_t adsMux){
  ...
  {
	  case (ADSMUXV):
		adsConfig = ADS1015_REG_CONFIG_MUX_DIFF_0_1 | adsConfig;
		break;
	  case (ADSMUXI):
		adsConfig = ADS1015_REG_CONFIG_MUX_DIFF_2_3 | adsConfig;
		break;
  }
	// start single shot conversion 
	writeRegister(m_i2cAddress, ADS1015_REG_POINTER_CONFIG, adsConfig | ADS1015_REG_CONFIG_OS_SINGLE);
