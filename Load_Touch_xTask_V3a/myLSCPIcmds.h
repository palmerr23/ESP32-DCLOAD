...
void printNumArgs(const char *tbuf, scpi_numeric output_numeric)
{
...
// return the ACTUAL voltage measurement mode 
scpi_error_t get_act_vmode(struct scpi_parser_context* context, struct scpi_token* command)
{ 
//Serial.print("SCPI get vmode actual\n");
  sprintf(SCPIoutBuf,"%c\n", _cur_vMode);
  SCPItrans(SCPIoutBuf);
  scpi_free_tokens(command);
  return SCPI_SUCCESS;
}
  
// return the  voltage measurement mode SETTING
scpi_error_t get_set_vmode(struct scpi_parser_context* context, struct scpi_token* command)
{ 
//Serial.print("SCPI get vmode\n");
  sprintf(SCPIoutBuf,"%c\n", pSet.vMode);
  SCPItrans(SCPIoutBuf);
  scpi_free_tokens(command);
  return SCPI_SUCCESS;
...
scpi_error_t set_vMode(struct scpi_parser_context* context, struct scpi_token* command)
{
  int i;
  struct scpi_token* args;
  args = command;
  ...
  //Serial.printf("SET MODE: [%s][%c]\n", args->value, args->value[0]);
  for(i = 0; i < C_VMODES; i++)
    if (args->value[0] == vModeNames[i])
    {
     pSet.vMode = args->value[0];  
     //Serial.print(pSet.vMode);
     valChanged(VAL_CHGD);
     dirtyScreen = true;
     break;
    } 
  if(i == C_VMODES)
  {
    ...
    strncpy(valBuf, args->value, args->length);
    valBuf[args->length] = '\0';
    sprintf(myError, "Command error; Invalid set voltage measurement mode '%s'", valBuf);
    error.description = myError;
    sprintf(SCPIoutBuf,"%i,\"%s\"", error.id,error.description);
    SCPItrans(SCPIoutBuf);
    scpi_queue_error(&ctx, error);
