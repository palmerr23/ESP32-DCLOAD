...
char * getUDP(){
  ...
  if (packetSize) 
  {
	...
	if(strncmp(SCpkt.id, "SCPL", 4)) // id chars not correct
	{
		//Serial.printf("UDP pkt doesn't start with SCPL\n");
		return NULL; 
	}
	/*
	if (SCpkt.grp != pSetX.trackGrp && SCpkt.grp != -1) //  my tracking group or global message
	{
		Serial.printf("UDP pkt not for my [%i] group = %i\n", pSetX.trackGrp, SCpkt.grp);
		return NULL;
	}
	*/
	// good packet	
